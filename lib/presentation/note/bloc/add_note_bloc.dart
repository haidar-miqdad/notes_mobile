...
class AddNoteBloc extends Bloc<AddNoteEvent, AddNoteState> {
  ...
  AddNoteBloc(
    ...
    on<AddNoteButtonPressed>((event, emit) async{
      emit(AddNoteLoading());
      final result = await remote.addNotes(event.title, event.content, event.isPin, event.image);
      result.fold(
        (message) => emit(AddNoteFailed(message: message)),
        (data) => emit(AddNoteSuccess(data: data)),
      );
     }
    );
  }
